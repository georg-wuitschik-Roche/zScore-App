...
_DEFAULT_REACTANT_OPTIONS = [
    ...
    and c in _default_df.columns
    and _default_df[c].notna().any()
]
...
def register(app):  # noqa: C901 – complexity is mostly decorator noise
    ...
    app.clientside_callback(
        ...
        [Input('settings-toggle', 'n_clicks'), Input('presentation-mode-toggle', 'n_clicks')],
    ...
    @app.callback(
        ...
        [State('upload-data', 'filename'), State('uploaded-data-store', 'data')],
        prevent_initial_call=True,
    )
        ...
        try:
            ...
            if len(decoded) > MAX_UPLOAD_BYTES:
                ...
                error_content = html.Div(
                    [
                        ...
                        html.P([html.Strong('Error: '), f'File size ({size_mb:.1f} MB) exceeds the 50 MB limit.']),
                    ]
                )
                return (
                    None,
            ...
            if df is None or len(df.columns) <= 1:
                error_content = html.Div(
                    [
                        html.P([html.Strong('File: '), filename]),
                        html.P(
                            [html.Strong('Error: '), 'Could not decode the file. The file encoding is not supported.']
                        ),
                        html.P('Please ensure your CSV file is saved with UTF-8 encoding.'),
                    ]
                )
                return (None, html.Span('Upload failed'), 'upload-status error', modal_visible, error_content)
            ...
            if missing_cols:
                ...
                error_content = html.Div(
                    [
                        html.P([html.Strong('File: '), filename]),
                        html.P([html.Strong('Error: '), f'Missing required columns: {", ".join(missing_cols)}']),
                        html.P([html.Strong('Columns found in your file:')]),
                        html.Code(
                ...
                return (None, html.Span('Missing columns'), 'upload-status error', modal_visible, error_content)
            ...
            if valid_zscores == 0:
                error_content = html.Div(
                    [
                        html.P([html.Strong('File: '), filename]),
                        html.P([html.Strong('Error: '), 'The z-Score column contains no valid numeric values.']),
                        html.P(
                            'Please ensure the z-Score column contains numeric values (decimals can use . or , as separator).'
                        ),
                    ]
                )
                return (None, html.Span('Invalid z-Score'), 'upload-status error', modal_visible, error_content)
            ...
            return (
                session_id,
                html.Span([html.I(className='fas fa-check-circle', style={'marginRight': '4px'}), status_text]),
            ...
            error_content = html.Div(
                [
                    html.P([html.Strong('File: '), filename if filename else 'Unknown']),
                    html.P([html.Strong('Error: '), 'An unexpected error occurred while processing your file.']),
    ...
    app.clientside_callback(
        ...
        [Input('upload-error-close', 'n_clicks'), Input('upload-error-close-btn', 'n_clicks')],
        prevent_initial_call=True,
    )
    ...
    # ------------------------------------------------------------------
    @app.callback(
        Output('reaction-type-dropdown', 'options'), [Input('uploaded-data-store', 'data'), Input('url', 'pathname')]
    )
    ...
    @app.callback(
        ...
        [Input('filter-trigger-store', 'data'), Input('url', 'pathname')],
        FILTER_STATES,
    )
    def _compute_stats(_trigger, pathname, *args):
    ...
    @app.callback(
        ...
        [Input('reaction-type-dropdown', 'value'), Input('uploaded-data-store', 'data'), Input('url', 'pathname')],
    )
    def _update_reactant_types_options(reaction_types, uploaded_data, pathname):
        ...
        if not reaction_types or len(reaction_types) == 0:
            ...
            filtered_categories = [
                c for c in du.CATEGORY_OPTIONS if c not in ['Functional Group A', 'Functional Group B']
            ]
    ...
    def _set_initial_fg_b_values(fg_b_options, current_fg_b):
        ...
        # Only set defaults if current values are truly empty (None or []), not if explicitly set to ['All']
        if not current_fg_b and fg_b_options:
    ...
    @app.callback(
        ...
        [Input('reset-btn', 'n_clicks'), Input('reaction-type-dropdown', 'value')],
        [
            State('reactant-types-dropdown', 'value'),
            State('functional-group-a-dropdown', 'options'),
            State('functional-group-b-dropdown', 'options'),
    ...
    def _reset_filters_and_update_min_eln(
        n_clicks, reaction_types, current_reactant_types, fg_a_options, fg_b_options, uploaded_data
    ):
        ...
        if triggered_id == 'reset-btn':
            ...
            if reaction_types and len(reaction_types) > 0:
                ...
                if 'Catalyst' in available_categories:
                    ...
                    default_reactant_types = (
                        [fallback_categories[0]] if fallback_categories else [du.CATEGORY_OPTIONS[0]]
                    )
            else:
                # Fallback to first non-excluded category
                fallback_categories = [
            ...
            return (
                ...
                default_fg_b,
                default_exclude_cui,
                default_exclude_scaleup,
                default_include_null_categories,
                default_min_eln,
                default_topn_zscore,
    ...
    # ------------------------------------------------------------------
    @app.callback(
        [Output('analysis-tabs', 'children'), Output('analysis-tabs', 'value')],
        [Input('reactant-types-dropdown', 'value')],
            ...
            dcc.Tab(
                ...
                children=[
                    html.Div(
                        ...
                        children=[dcc.Graph(id='boxplot', clear_on_unhover=True, style={'height': '100%'})],
                    )
                ],
            )
        ]
                ...
                dcc.Tab(
                    ...
                    children=[
                        html.Div(
                            ...
                            style={'height': '800px', 'width': '100%'},
                            children=[dcc.Graph(id='heatmap', clear_on_unhover=True, style={'height': '100%'})],
                        )
                    ],
                )
            )
    ...
    @app.callback(
        ...
        [Input('filter-trigger-store', 'data'), Input('analysis-tabs', 'value'), Input('url', 'pathname')],
        FILTER_STATES,
    )
    def _update_stats_table(_trigger, active_tab, pathname, *args):
        if pathname != '/dashboard':
            return []
        ...
        header = html.Div(
            f'Rows: {len(dff):,} | Unique ELNs: {eln_count:,}', style={'marginBottom': '8px', 'color': '#6c757d'}
        )
    ...
    @app.callback(
        ...
        [State('max-components-input', 'value'), State('max-components-input', 'max')],
        prevent_initial_call='initial_duplicate',
    )
    def _update_max_components_slider(stats, current_value, previous_max):
        ...
        if current_value is None:
            ...
            and isinstance(previous_max, int | float)
            and max_value > previous_max
            and current_value == previous_max
        ):
            ...
            and isinstance(previous_max, int | float)
            and max_value > previous_max
            and current_value < default_target
        ):
    ...
    app.clientside_callback(
        ...
        [Input('filter-stats-store', 'data'), Input('reactant-types-dropdown', 'value')],
    )
    ...
    # ------------------------------------------------------------------
    _PLOT_INPUTS = [
        Input('filter-trigger-store', 'data'),
        Input('analysis-tabs', 'value'),
        Input('presentation-mode-store', 'data'),
        Input('url', 'pathname'),
    ]

    def _update_plot(plot_fn, expected_tab, _trigger, active_tab, presentation_mode, pathname, *filter_args):
        ...
        fkw = _parse_filter_args(filter_args)
        reactant_types = fkw['reactant_types'] or DEFAULT_REACTANT_FALLBACK
        fkw['reactant_types'] = reactant_types
        ...
        container_style = {'height': f'{container_height}px', 'width': '100%'}
        return fig, container_style

    @app.callback(
        [Output('boxplot', 'figure'), Output('boxplot-container', 'style')],
        _PLOT_INPUTS,
        FILTER_STATES,
    )
    ...
    # ------------------------------------------------------------------
    @app.callback(
        [Output('heatmap', 'figure'), Output('heatmap-container', 'style')],
        _PLOT_INPUTS,
        FILTER_STATES,
    )
    ...
    def _tutorial_state(
        ...
        _step_kw = dict(
            ...
            fg_a_vals=fg_a_vals,
            fg_b_vals=fg_b_vals,
            filter_panel_style=filter_panel_style,
            min_eln=min_eln,
            topn=topn,
            max_comp=max_comp,
            exclude_cui_val=exclude_cui_val,
        ...
        if data.get('active'):
            ...
            if trigger_id in step_to_triggers.get(current_step, []) and _is_tutorial_step_satisfied(
                current_step, **_step_kw
            ):
