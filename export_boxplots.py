...
REACTANT_CATEGORIES: list[str] = [
    cat for cat in du.CATEGORY_OPTIONS if cat not in ['Functional Group A', 'Functional Group B']
]
...
def clean_directory(path: Path, message: str | None = None) -> None:
    ...
    for i, png_file in enumerate(png_files, start=2):
        ...
        if len(parts) >= 3:
            if parts[0] == 'Buchwald-Hartwig' and len(parts) >= 4:
                ...
                description = (
                    f'{category} boxplot for {reaction} reactions for {functional_group} reacting with aryl halides'
                )
            else:
