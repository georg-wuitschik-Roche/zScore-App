...
# Path to the cleaned data export — look in repo root (one level up from paper/)
_PAPER_DIR = Path(__file__).parent
_REPO_ROOT = _PAPER_DIR.parent
local_csv = _REPO_ROOT / 'z-Score Peaks with FG.csv'
cloud_csv = Path('zscore_peaks_data.csv')

if local_csv.exists():
    CSV_PATH = local_csv
    logger.info('Using CSV from repo root: %s', CSV_PATH)
else:
    CSV_PATH = cloud_csv
    ...
    try:
        ...
        try:
            try:
                ...
                logger.warning(
                    'google-cloud-storage not installed or unavailable; skipping authenticated GCS download: %s',
                    import_error,
                )
                return False
    ...
    try:
        ...
        for encoding in ['utf-8-sig', 'latin-1', 'cp1252', 'iso-8859-1']:
            try:
                ...
                logger.info(
                    'CSV validation successful with %s encoding - found %d columns', encoding, len(test_df.columns)
                )
                return True
            except Exception:
                continue
...
def _read_csv_with_encoding(csv_path: Path) -> pd.DataFrame:
    ...
    for encoding in encodings_to_try:
        try:
            ...
            logger.info(
                'Successfully read CSV with %s encoding - %d rows, %d columns', encoding, len(df), len(df.columns)
            )
            return df
        except UnicodeDecodeError as e:
    ...
    raise UnicodeDecodeError(
        'utf-8', b'', 0, 1, f'Could not read CSV file with any of the attempted encodings: {encodings_to_try}'
    )
...
def _load_and_prepare(csv_path: Path = CSV_PATH) -> pd.DataFrame:
    ...
    df['z-Score'] = df['z-Score'].astype(str).str.replace(',', '.').str.strip().pipe(pd.to_numeric, errors='coerce')
...
def _filter_by_reactant_columns(dff: pd.DataFrame, reactant_types: list | None, include_null: bool) -> pd.DataFrame:
...
def _filter_fg_b(dff: pd.DataFrame, fg_b: str | list | None, fg_a_list: list[str]) -> tuple[pd.DataFrame, list[str]]:
...
def _filter_scaleup_plates(dff: pd.DataFrame, exclude_scaleup: list | None) -> pd.DataFrame:
    ...
    plate_variability = dff.groupby(['ELN_ID', 'PLATENUMBER'])[reagent_cols].nunique()
...
def _filter_max_components(
    ...
    medians = dff.groupby(key_cols, dropna=not include_null)['z-Score'].median().sort_values(ascending=False)
...
def _cleanup_expired_uploads() -> None:
    ...
    """
    now = time.time()
    expired = [sid for sid, entry in _UPLOAD_STORE.items() if now - entry['last_access'] > _UPLOAD_TTL_SECONDS]
    for sid in expired:
...
def compute_distribution_stats(
    dff: pd.DataFrame, group_col: str = 'Reaction Type', value_col: str = 'z-Score', min_samples: int = 20
) -> pd.DataFrame:
...
def compute_significance_tests(
    dff: pd.DataFrame, category_col: str, value_col: str = 'z-Score', top_n: int = 10, alpha: float = 0.05
) -> dict:
    ...
    result = {'n_groups': len(groups), 'group_stats': pd.DataFrame(group_stats) if group_stats else pd.DataFrame()}
