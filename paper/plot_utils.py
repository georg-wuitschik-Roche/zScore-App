...
def _apply_common_layout(fig: go.Figure, *, title: str, fs: dict, height: int, margin: dict | None = None) -> None:
    """Apply the shared layout styling (background, title, font)."""
    fig.update_layout(
        title=dict(text=title, font=dict(size=fs['title'], family=_FONT_FAMILY, color=_TITLE_COLOR)),
...
def _add_stats_annotation(fig: go.Figure, text: str, fs: dict, x: float = 0.98, align: str = 'right') -> None:
...
def create_boxplot(
    ...
    dff_hover['hover_text'] = (
        ...
        + '<b>Reaction:</b><br>'
        + 'Reaction Type: '
        + _col_str('Reaction Type')
        + '<br>'
        + '<br>'
        + '<b>Reaction Conditions:</b><br>'
        + _col_str('output_column')
        + '<br>'
        + '<br>'
        + '<b>Reagents:</b><br>'
        + 'Catalyst: '
    ...
    fig.update_traces(hovertemplate=hover_template, hoverinfo='all')
...
def create_heatmap(
    dff, reactant_types: list, base_height: int = 800, presentation_mode: bool = False
) -> tuple[go.Figure, int]:
    ...
    if x_category:
        ...
        if len(valid_data) > 0:
            ...
            colorscale = [[0, 'blue'], [(zmid - zmin) / (zmax - zmin), 'white'], [1, 'red']]
        else:
        ...
        fig = go.Figure(
            data=go.Heatmap(
                ...
                colorbar=dict(
                    title=dict(text='Median z-Score', font=dict(size=fs['colorbar_title'], family=_FONT_FAMILY)),
    ...
    fig.update_layout(
        xaxis=dict(
            title=dict(text=x_axis_title, font=dict(size=fs['axis_title'], weight='bold', family=_FONT_FAMILY)),
        ...
        yaxis=dict(
            title=dict(text=reactant_types[0], font=dict(size=fs['axis_title'], weight='bold', family=_FONT_FAMILY)),
            tickfont=dict(size=fs['tick'], weight='bold', family=_FONT_FAMILY),
            showgrid=False,
            zeroline=False,
            showline=True,
...
def create_distribution_plot(
    ...
    """
    data, default_title = _filter_diagnostic_data(dff, value_col, group_col, group_value, 'Distribution')
    ...
    _apply_common_layout(fig, title=title, fs=fs, height=_DIAGNOSTIC_PLOT_HEIGHT, margin=dict(l=60, r=60, t=80, b=60))
    fig.update_layout(
...
def create_qq_plot(
    ...
    Returns:
        Tuple of (figure, height)
    """
    data, default_title = _filter_diagnostic_data(dff, value_col, group_col, group_value, 'Q-Q Plot')
    title = title or default_title
    fs = _font_sizes(presentation_mode, 'diagnostic')
...
def create_distribution_summary_table(dist_stats: pd.DataFrame, presentation_mode: bool = False) -> go.Figure:
    ...
    if dist_stats.empty:
        fig = go.Figure()
        fig.add_annotation(
            text='No distribution statistics available', xref='paper', yref='paper', x=0.5, y=0.5, showarrow=False
        )
        return fig
    ...
    if 'is_normal' in display_df.columns:
        display_df['is_normal'] = display_df['is_normal'].apply(lambda x: '✓' if x else ('✗' if not x else '?'))
    ...
    _apply_common_layout(
        fig, title='Distribution Statistics by Reaction Type', fs=fs, height=height, margin=dict(l=20, r=20, t=60, b=20)
    )

    return fig


def create_significance_summary_table(sig_results: dict, presentation_mode: bool = False) -> go.Figure:
        ...
        [
            {
                ...
                'Statistic': f'{kw.get("statistic", "N/A"):.2f}'
                if isinstance(kw.get('statistic'), int | float)
                else 'N/A',
                'p-value': f'{kw.get("p_value", "N/A"):.2e}'
                if isinstance(kw.get('p_value'), int | float) and kw.get('p_value', 1) < 0.001
                else (f'{kw.get("p_value", "N/A"):.4f}' if isinstance(kw.get('p_value'), int | float) else 'N/A'),
    ...
    _hdr = dict(
        fill_color=_TABLE_HEADER_COLOR, font=dict(color='white', size=fs['header'], family=_FONT_FAMILY), align='center'
    )
    ...
    fig.add_trace(
        go.Table(
            ...
            cells=dict(
                values=[kw_data[c] for c in kw_data.columns], fill_color='white', font=_cell_font, align='center'
            ),
        ),
        row=1,
        col=1,
    ...
    if not group_stats.empty:
        fig.add_trace(
            go.Table(
                header=dict(values=['<b>' + c.replace('_', ' ').title() + '</b>' for c in group_stats.columns], **_hdr),
                cells=dict(
                    values=[group_stats[c] for c in group_stats.columns],
    ...
    if not pairwise.empty:
        ...
        if 'significant' in pairwise_display.columns:
            pairwise_display['significant'] = pairwise_display['significant'].apply(lambda x: '✓' if x else '✗')

        fig.add_trace(
            go.Table(
                header=dict(values=['<b>' + c.replace('_', ' ').title() + '</b>' for c in display_cols], **_hdr),
                cells=dict(
                    values=[pairwise_display[c] for c in display_cols],
    ...
    _apply_common_layout(
        fig, title='Statistical Significance Analysis', fs=fs, height=800, margin=dict(l=20, r=20, t=80, b=20)
    )
