/**
 * Tutorial overlay — 23-step guided walkthrough.
 *
...
// Steps 5-13 target elements inside the options panel
const STEPS_REQUIRING_PANEL_OPEN = new Set([5, 6, 7, 8, 9, 10, 11, 12, 13]);

// Steps 18-20 are settings sub-panels, step 21 is reset — all in navbar
const STEPS_IN_NAVBAR = new Set([18, 19, 20, 21]);

// Steps 18-20 show settings modal side-by-side with tutorial
const SETTINGS_STEPS = new Set([18, 19, 20]);
...
export function TutorialOverlay() {
  ...
  // Step 16 — set reactants to Catalyst + Base + Solvent, show violin, enable split
  useEffect(() => {
    if (!active || step !== 16) return;
    setActiveTab('violin');
  ...
  }, [active, step, reactantTypes, availableReactants, setReactantTypes, setActiveTab, setSplitSelector]);

  // Steps 18-20 — auto-open settings modal, keep open across sub-steps
